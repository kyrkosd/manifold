...
# ---------------------------------------------------------------------------
# Module-level helpers (not part of class WMC)
# ---------------------------------------------------------------------------

def _load_coefficients(run_dir: Path) -> np.ndarray:
    """Load coefficients.npy; fall back to random 3-D positions if absent."""
...
def _load_power_spectrum(run_dir: Path) -> np.ndarray | None:
...
def _load_anomaly(run_dir: Path) -> dict | None:
...
def _load_manifold(run_dir: Path) -> dict | None:
...
def _load_cluster_labels(run_dir: Path) -> np.ndarray | None:
    """Load cluster_labels.npy; returns None if absent."""
...
def _load_column_names(run_dir: Path) -> list[str] | None:
    """Load column_names.json; returns None if absent."""
...
def _project_to_3d(coeff: np.ndarray, axes: list[int]) -> np.ndarray:
    """Project coefficients onto axes, zero-mean unit-variance per axis."""
...
def _check_cluster_overlap(
    ...
    """Return True if the cluster centroid is within threshold × mean spread of normal points."""
...
def _find_divergent_axes(
    ...
    """Return the 3 axes with the largest power difference between cluster and normal points."""
...
def _reproject_cluster(
    ...
    """Normalise cluster onto divergent axes and shift it away from the main surface."""
...
def _build_clusters(
    ...
    for cid, cidx in regional.items():
        ...
        if (
            len(normal_idx) >= 5
            and _check_cluster_overlap(cluster_pos, normal_pos)
        ):
            div_axes = _find_divergent_axes(cidx, normal_idx, coeff)
            cluster_pos = _reproject_cluster(
                cidx,
                coeff,
                div_axes,
...
def _get_chart_info(manifold: dict, index: int) -> tuple[int, float]:
...
class ManifoldProjector:
    ...
    def project(self, run_dir: Path) -> ProjectionResult:
        ...
        coeff = _load_coefficients(run_dir)
        power = _load_or_compute_power(run_dir, coeff)
        anomaly = _load_or_default_anomaly(run_dir, len(coeff))
        cluster_labels = _load_cluster_labels(run_dir)

        axes = self.select_top3_axes(power)
        positions_all = _project_to_3d(coeff, axes)

        flags = np.array(anomaly.get("flags", [False] * len(coeff)), dtype=bool)
        scores = np.array(anomaly.get("scores", [0.0] * len(coeff)), dtype=float)

        normal_idx, isolated_idx, regional = _classify_points(flags, cluster_labels)
        clusters = _build_clusters(regional, positions_all, normal_idx, coeff)
    ...
    def point_detail(self, run_dir: Path, index: int) -> PointDetailResponse:
        ...
        anomaly = _load_or_default_anomaly(run_dir, len(coeff))
        manifold = _load_or_default_manifold(run_dir, len(coeff))
        col_names = _load_column_names(run_dir)
        ...
        chart_id, chart_quality = _get_chart_info(manifold, index)
        orig = _get_original_values(coeff, index, col_names)
